...
for (s in 1:length(species)) {
  for (m in metric) {
    file <- paste0("Simulation Results ", species[s], " ", m,".csv")
    sim_results <-
      read.csv(file) 
    
    observed_results <- read.csv("../Gut Analysis/Species Prey size summary.csv")
    ...
    write.csv(results_summary,
              paste0(species_names[s], "P_values_", m, ".csv"))
